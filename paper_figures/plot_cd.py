...
# This file does not subject to the license of the project
...
def draw_cd_diagram(df_perf=None, alpha=0.05, title=None, labels=False):
    ...
    p_values, average_ranks, _ = nemenyi_two_tailed(df_perf=df_perf, alpha=alpha)
...
def nemenyi_two_tailed(alpha=0.05, df_perf=None):
    """
    Nemenyi two tailed rank test.
    """
    print(pd.unique(df_perf['classifier_name']))
    # count the number of tested datasets per classifier
    df_counts = pd.DataFrame({'count': df_perf.groupby(
    ...
    for i in range(m - 1):
        ...
        for j in range(i + 1, m):
            ...
            perf_2 = np.array(df_perf.loc[df_perf['classifier_name'] == classifier_2]
                              ['accuracy'], dtype=np.float64)
            # print(perf_1, perf_2)
            test_data = np.array([perf_1, perf_2])
            # print(test_data.shape)
            result = sp.posthoc_nemenyi_friedman(test_data.T)
            # result = sp.posthoc_nemenyi(test_data.T)
            # print(result)
...
for metric in ['ari', 'nmi', 'purity']:
    ...
    if metric == 'ari':
        ...
        draw_cd_diagram(df_perf=df_perf, title='NMI Rank', labels=True, alpha=0.05)
    elif metric == 'purity':
        draw_cd_diagram(df_perf=df_perf, title='Purity Rank', labels=True, alpha=0.05)